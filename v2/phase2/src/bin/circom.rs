...
use bellman_ce::groth16::{create_random_proof, prepare_verifying_key, verify_proof, Proof};
...
impl<'a, E: Engine> Circuit<E> for CircomCircuit<'a> {
    fn synthesize<CS: ConstraintSystem<E>>(self, cs: &mut CS) -> Result<(), SynthesisError> {
        ...
        for (i, constraint) in circuit_json.constraints.iter().enumerate() {
            ...
            for lc_description in constraint {
                ...
                for (var_index_str, coefficient_str) in lc_description {
                    ...
                    if i == 2 {
                        lc = lc
                            + (
                                E::Fr::from_str(coefficient_str).unwrap(),
                                Variable::new_unchecked(var_index),
                            );
                    } else {
                        lc = lc
                            + (
            ...
            cs.enforce(
                || format!("constraint {}", constraint_num),
                |_| lcs[0].clone(),
                |_| lcs[1].clone(),
                |_| lcs[2].clone(),
            );
...
fn main() {
    ...
    let file_name = "subcircuit012_constrained.json";
    ...
    let mut mmap = unsafe { memmap::Mmap::map(&pk_file) }
        .unwrap()
        .make_mut()
        .unwrap();
    ...
    let mut mmap = unsafe { memmap::Mmap::map(&vk_file) }
        .unwrap()
        .make_mut()
        .unwrap();
