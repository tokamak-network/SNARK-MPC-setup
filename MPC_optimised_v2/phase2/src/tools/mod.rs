...
// // use icicle_bls12_381::polynomials::{DensePolynomialExt, BivariatePolynomial};
// use icicle_core::polynomials::UnivariatePolynomial;
// use icicle_bls12_381::curve::{ScalarField, ScalarCfg};
// use icicle_bls12_381::polynomials::{DensePolynomial, UnivariatePolynomial};
// use icicle_core::polynomials::UnivariatePolynomial;
...
// Import necessary polynomial structures
use icicle_bls12_381::curve::{ScalarField, ScalarCfg};
...
impl MixedSubcircuitQAPEvaled {
    pub fn from_r1cs_to_evaled_qap(
        ...
        // for (i, _) in self.evaled_qap.a_evals.iter().enumerate() {
        // for (i, _) in evaled_qap.a_evals.iter().enumerate() {
            for (i, _) in qap_polys.u_evals.iter().enumerate() {
            ...
            for val in mul_res_vec.iter() {
                // sum = sum + val;
                sum = sum + *val;
            }
            // u_evals_long[*wire_idx] = sum;
            // for (i, val) in something.iter().enumerate() {
            //     sum = sum + val;
            //     // u_evals_long[*wire_idx] = sum; // incorrect
            //     u_evals_long[i] = sum;
            // }
            let mut sum = ScalarField::zero();
for val in mul_res_vec.iter() {
...
pub fn gen_cached_pows(val: &ScalarField, size: usize, res: &mut Box<[ScalarField]>) {
    ...
    // let temp_poly = DensePolynomial::from_rou_evals(temp_evals, size, 1, None, None);
    let temp_poly = DensePolynomial::from_rou_evals(temp_evals, size);
