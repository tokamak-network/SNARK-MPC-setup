...
fn main() {
    ...
    let current_accumulator = BachedAccumulator::<Bls12, Bls12CeremonyParameters>::deserialize(
    ...
    for m in 0..max_degree {
        ...
        let mut g1_delta_coeffs = EvaluationDomain::from_coeffs(
            current_accumulator.delta_tau_powers_g1[0..degree].iter()
                .map(|e| Point(e.into_projective()))
                .collect()
        ).unwrap();
        ...
        let mut g1_delta_coeffs = g1_delta_coeffs.into_iter()
            .map(|e| e.0)
            .collect::<Vec<_>>();
        ...
        G1::batch_normalization(&mut g1_delta_coeffs);
        ...
        // Write alpha (in g1)
        // Needed by verifier for e(alpha, delta)
        ...
        writer.write_all(
            current_accumulator.delta_tau_powers_g1[0]
                .into_uncompressed()
                .as_ref()
        ).unwrap();
        ...
        // Lagrange coefficients in G1 with delta (for
        // LC/IC queries)
        for coeff in g1_delta_coeffs {
            // Was normalized earlier in parallel
            let coeff = coeff.into_affine();
