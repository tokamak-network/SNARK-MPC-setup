...
impl PowersOfTauParameters for Bls12CeremonyParameters {
    ...
    // const EMPIRICAL_BATCH_SIZE: usize = 5;  // güvenli ve tutarlı değer
...
const TAU_POWERS_LENGTH: usize = 1 << 4;
...
pub const ACCUMULATOR_BYTE_SIZE: usize = (TAU_POWERS_G1_LENGTH * G1_UNCOMPRESSED_BYTE_SIZE) + // g1 tau powers
                                         ...
                                         + G2_UNCOMPRESSED_BYTE_SIZE // delta in g2
                                         + 64; // blake2b hash of previous contribution

/// The "public key" is used to verify a contribution was correctly
/// computed.
pub const PUBLIC_KEY_SIZE: usize = 3 * G2_UNCOMPRESSED_BYTE_SIZE + // tau, alpha, and delta in g2
                                   6 * G1_UNCOMPRESSED_BYTE_SIZE; // (s1, s1*tau), (s2, s2*alpha), (s3, s3*delta) in g1
...
pub const CONTRIBUTION_BYTE_SIZE: usize = (TAU_POWERS_G1_LENGTH * G1_COMPRESSED_BYTE_SIZE) + // g1 tau powers
                                          ...
                                          (TAU_POWERS_LENGTH * G1_COMPRESSED_BYTE_SIZE) // delta tau powers
                                          + G2_COMPRESSED_BYTE_SIZE // delta in g2
...
impl PublicKey {
    ...
    pub fn serialize<W: Write>(&self, writer: &mut W) -> io::Result<()>
    {
        ...
        write_point(writer, &self.delta_g1.0, UseCompression::No)?;
        write_point(writer, &self.delta_g1.1, UseCompression::No)?;

        write_point(writer, &self.tau_g2, UseCompression::No)?;
        write_point(writer, &self.alpha_g2, UseCompression::No)?;
        write_point(writer, &self.delta_g2, UseCompression::No)?;
    ...
    pub fn deserialize<R: Read>(reader: &mut R) -> Result<PublicKey, DeserializationError>
    {
        ...
        let delta_g1_s = read_uncompressed(reader)?;
        let delta_g1_s_delta = read_uncompressed(reader)?;

        let tau_g2 = read_uncompressed(reader)?;
        let alpha_g2 = read_uncompressed(reader)?;
        let delta_g2 = read_uncompressed(reader)?;

        Ok(PublicKey {
            tau_g1: (tau_g1_s, tau_g1_s_tau),
            alpha_g1: (alpha_g1_s, alpha_g1_s_alpha),
            delta_g1: (delta_g1_s, delta_g1_s_delta),
            tau_g2: tau_g2,
            alpha_g2: alpha_g2,
            delta_g2: delta_g2
        })
...
pub struct Accumulator {
    ...
    pub delta_tau_powers_g1: Vec<G1Affine>,
    /// delta
...
impl Accumulator {
    ...
    pub fn new() -> Self {
        Accumulator {
            ...
            delta_tau_powers_g1: vec![G1Affine::one(); TAU_POWERS_LENGTH],
    ...
    ) -> io::Result<()>
    {
        ...
        write_all(writer, &self.delta_tau_powers_g1, compression)?;
        write_all(writer, &[self.delta_g2], compression)?;
    ...
    ) -> Result<Self, DeserializationError>
    {
        ...
        let delta_tau_powers_g1 = read_all(reader, TAU_POWERS_LENGTH, compression, checked)?;
        let delta_g2 = read_all(reader, 1, compression, checked)?[0];
    ...
    pub fn transform(&mut self, key: &PrivateKey)
    {
        ...
        batch_exp(&mut self.delta_tau_powers_g1, &taupowers[0..TAU_POWERS_LENGTH], Some(&key.delta));
        self.delta_g2 = self.delta_g2.mul(key.delta).into_affine();
    }
}
...
pub fn verify_transform(before: &Accumulator, after: &Accumulator, key: &PublicKey, digest: &[u8]) -> bool
{
    ...
    let delta_g2_s = compute_g2_s(key.delta_g1.0, key.delta_g1.1, 2);
