...
pub struct PublicKey<E: Engine> {
    ...
    pub delta_g1: (E::G1Affine, E::G1Affine),
...
impl<E: Engine> PartialEq for PublicKey<E> {
    fn eq(&self, other: &PublicKey<E>) -> bool {
        ...
        self.delta_g1.0 == other.delta_g1.0 &&
        self.delta_g1.1 == other.delta_g1.1 &&
...
pub fn keypair<R: Rng, E: Engine>(rng: &mut R, digest: &[u8]) -> (PublicKey<E>, PrivateKey<E>)
{
    ...
    // alpha and delta are a set of conrtibuitons in a form "alpha * tau^i * G" and that are required
...
impl<E: Engine> PublicKey<E> {
    ...
    pub fn deserialize<R: Read>(reader: &mut R) -> Result<PublicKey<E>, DeserializationError>
    {
        ...
        let delta_g1_s = read_uncompressed::<E, _, _>(reader)?;
        let delta_g1_s_delta = read_uncompressed::<E, _, _>(reader)?;

        let tau_g2 = read_uncompressed::<E, _, _>(reader)?;
        let alpha_g2 = read_uncompressed::<E, _, _>(reader)?;
        let delta_g2 = read_uncompressed::<E, _, _>(reader)?;
...
impl<E: Engine> PublicKey<E> {
        ...
        where P: PowersOfTauParameters
    {
        ...
        (&mut output_map[position..]).write(&self.delta_g1.0.into_uncompressed().as_ref())?;
        position += P::G1_UNCOMPRESSED_BYTE_SIZE;

        (&mut output_map[position..]).write(&self.delta_g1.1.into_uncompressed().as_ref())?;
        position += P::G1_UNCOMPRESSED_BYTE_SIZE;
        ...
        (&mut output_map[position..]).write(&self.delta_g2.into_uncompressed().as_ref())?;
        ...
        where P: PowersOfTauParameters
    {
        ...
        let delta_g1_s = read_uncompressed::<E, _>(input_map, position)?;
        position += P::G1_UNCOMPRESSED_BYTE_SIZE;

        let delta_g1_s_delta = read_uncompressed::<E, _>(input_map, position)?;
        position += P::G1_UNCOMPRESSED_BYTE_SIZE;
        ...
        let delta_g2 = read_uncompressed::<E, _>(input_map, position)?;
