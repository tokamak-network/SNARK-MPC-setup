...
pub trait PowersOfTauParameters: Clone {
    ...
    const COMPRESSED_CHALLENGE_LENGTH: usize; 
    const NEW_CHALLENGE_LENGTH: usize;         
    ...
    const ACCUMULATOR_BYTE_SIZE: usize = (Self::TAU_POWERS_G1_LENGTH * Self::G1_UNCOMPRESSED_BYTE_SIZE) + // g1 tau powers
                                            ...
                                            (Self::TAU_POWERS_LENGTH * Self::G1_UNCOMPRESSED_BYTE_SIZE) // delta tau powers
                                            + Self::G2_UNCOMPRESSED_BYTE_SIZE // delta in g2
                                            + Self::HASH_SIZE; // blake2b hash of previous contribution

    const PUBLIC_KEY_SIZE: usize = 3 * Self::G2_UNCOMPRESSED_BYTE_SIZE + // tau, alpha, and delta in g2
                                    6 * Self::G1_UNCOMPRESSED_BYTE_SIZE; // (s1, s1*tau), (s2, s2*alpha), (s3, s3*delta) in g1
    ...
    const CONTRIBUTION_BYTE_SIZE: usize = (Self::TAU_POWERS_G1_LENGTH * Self::G1_COMPRESSED_BYTE_SIZE) + // g1 tau powers
                                            ...
                                            (Self::TAU_POWERS_LENGTH * Self::G1_COMPRESSED_BYTE_SIZE) // delta tau powers
                                            + Self::G2_COMPRESSED_BYTE_SIZE // delta in g2
