...
fn main() {
    ...
    // Create `./response2` in this directory
    let writer = OpenOptions::new()
        .read(false)
        .write(true)
        .create_new(true)
        .open("response2")
        .expect("unable to create `./response2` in this directory");
    ...
    println!("Computing, this could take a while...");
    current_accumulator.transform(&priv_key);
    println!("Writing your contribution to `./response2`...");
    ...
    print!(
        "Done!\n\n\
          Your contribution has been written to `./response2`\n\n\
          The BLAKE2b hash of `./response2` is:\n"
    );
