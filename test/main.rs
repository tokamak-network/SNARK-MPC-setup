...
struct QAPPolynomialsJson {
    u_active_wires: Vec<usize>,
    v_active_wires: Vec<usize>,
    w_active_wires: Vec<usize>,
    u_evals: Vec<Vec<String>>,
    v_evals: Vec<Vec<String>>,
    w_evals: Vec<Vec<String>>,
}

fn scalar_to_hex(scalar: &ScalarField) -> String {
    scalar.to_bytes_le().iter().map(|b| format!("{:02x}", b)).collect()
}

fn write_qap_polynomials_json(qap: &MixedSubcircuitQAPEvaled, file_path: &str) {
    let json_data = QAPPolynomialsJson {
        u_active_wires: qap.active_wires.iter().copied().collect(),
        v_active_wires: qap.active_wires.iter().copied().collect(),
        w_active_wires: qap.active_wires.iter().copied().collect(),
        u_evals: vec![qap.o_evals.iter().map(scalar_to_hex).collect()],
        v_evals: vec![qap.o_evals.iter().map(scalar_to_hex).collect()],
        w_evals: vec![qap.o_evals.iter().map(scalar_to_hex).collect()],
    };

    let json_string = serde_json::to_string_pretty(&json_data).expect("JSON conversion is unsuccessful");
    let mut file = File::create(file_path).expect("File cannot be created!");
    file.write_all(json_string.as_bytes()).expect("Dosyaya yazılamadı!");

    println!("QAP polinomials generated successfully: {}", file_path);
}
...
// ---------------------------------------------------------------------------------
// --------------------Other additions for MPC setup --------------------------------------------
// ---------------------------------------------------------------------------------
let global_wire_list_path = "setup/trusted-setup/inputs/globalWireList.json";
let setup_params = SetupParams::from_path("setup/trusted-setup/inputs/setupParams.json").unwrap();
let subcircuit_infos = SubcircuitInfo::from_path("setup/trusted-setup/inputs/subcircuitInfo.json").unwrap();

let tau = Tau::gen(); 

let cached_x_pows_vec: Box<[ScalarField]> = Box::new([
    ScalarField::zero(), // Buraya uygun cached_x_pows değerlerini yaz
    ScalarField::one(), 
    // Burada gerçek cached_x_pows dizini kullanmalısın!
]);

// This line corrects the error:
let subcircuit_json_path = "setup/trusted-setup/inputs/json/subcircuit0.json";

let mixed_evaled_qap = MixedSubcircuitQAPEvaled::from_r1cs_to_evaled_qap(
    subcircuit_json_path,     // ✅ correct file is generated here
    &setup_params,
    &subcircuit_infos[0],
    &tau,
    &cached_x_pows_vec
...
// Save QAP polynomials as JSON file
write_qap_polynomials_json(&mixed_evaled_qap, "full_circuit_qap.json");
// ---------------------------------------------------------------------------------
// ---------------------------------------------------------------------------------
// ---------------------------------------------------------------------------------
}
