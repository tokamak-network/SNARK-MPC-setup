use ark_bls12_381::{G1Affine as ArkG1Affine, G1Projective as ArkG1Projective, G2Affine as ArkG2Affine, G2Projective as ArkG2Projective};
...
use icicle_bls12_381::curve::{G1Affine as IcicleG1Affine, G1Projective as IcicleG1Projective, G2Affine as IcicleG2Affine, G2Projective as IcicleG2Projective, ScalarField};
...
pub fn serialize_g1_affine_compressed(point: &IcicleG1Affine) -> [u8; 48] {
    let mut buf = Vec::new();
...
fn ark_to_icicle_g1projective_points(ark_projective: &[ArkG1Projective]) -> Vec<IcicleG1Projective> {
    ark_projective
        .par_iter()
        .map(|ark| {
...
fn ark_to_icicle_g2projective_points(ark_projective: &[ArkG2Projective]) -> Vec<IcicleG2Projective> {
    ark_projective
        .par_iter()
        .map(|ark| {
            let proj_x = ark.x * ark.z;
...
fn test_g2serde_serialization_roundtrip() {
    ...
    let serialized = serde_json::to_string(&original_points)
        .expect("Serialization of Vec<G2serde> failed");

    println!("Serialized Vec<G2serde>: {}", serialized);

    // Deserialize JSON back into Vec<G2serde>
    let deserialized: Vec<G2serde> = serde_json::from_str(&serialized)
        .expect("Deserialization of Vec<G2serde> failed");
